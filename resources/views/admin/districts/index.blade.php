...
<main class="flex-1 p-6 md:p-10 bg-gradient-to-br from-gray-50 to-gray-100">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-4xl font-bold text-gray-900 mb-2">Districts</h1>
                <p class="text-gray-600">Manage and organize district information</p>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div class="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-[#d71418]/5 to-[#f19e00]/5">
            <div class="flex items-center">
                ...
                <div>
                    <h2 class="text-xl font-semibold text-gray-900">District List</h2>
                    <p class="text-gray-600 text-sm">View and manage all districts in the system</p>
                </div>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-all duration-200">
                        <td class="px-8 py-6 whitespace-nowrap text-sm font-medium text-gray-900">{{ $district->id }}</td>
                        <td class="px-8 py-6 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <span class="font-semibold text-gray-900">{{ $district->name }}</span>
                            </div>
                        </td>
                        <td class="px-8 py-6 whitespace-nowrap text-sm text-gray-600">{{ $district->province->name ?? 'N/A' }}</td>
                        <td class="px-8 py-6 whitespace-nowrap text-sm text-gray-600">{{ $district->zip_code ?? 'NULL' }}</td>
                        <td class="px-8 py-6 whitespace-nowrap">
                            @if($district->is_established)
                        ...
                        <td class="px-8 py-6 whitespace-nowrap text-sm text-gray-600">{{ $district->created_at ? $district->created_at->format('Y-m-d H:i:s') : 'N/A' }}</td>
                        <td class="px-8 py-6 whitespace-nowrap">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('admin.districts.edit', $district) }}" class="inline-flex items-center px-4 py-2 bg-[#d71418] text-white text-xs font-semibold rounded-lg hover:bg-[#b31216] transition-all duration-200 shadow-sm hover:shadow-md">
                                    <i class="fas fa-edit mr-1"></i> Edit
                                </a>
                                <form method="POST" action="{{ route('admin.districts.destroy', $district) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirm('Are you sure you want to delete this district?')" class="inline-flex items-center px-4 py-2 bg-gray-500 text-white text-xs font-semibold rounded-lg hover:bg-gray-600 transition-all duration-200 shadow-sm hover:shadow-md">
                    ...
                    <tr>
                        <td colspan="7" class="px-8 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-gray-400 text-sm mt-1">Get started by adding your first district</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
...
<!-- Add District Modal -->
<div id="addDistrictModal" class="fixed inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
        ...
        <div class="mt-3">
            ...
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center">
                    ...
                    <h3 class="text-xl font-semibold text-gray-900">Add New District</h3>
                </div>
                ...
                <div class="space-y-6">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-3" for="name">
                            <i class="fas fa-city mr-2 text-[#d71418]"></i>
                            District Name
                        </label>
                        <input type="text" name="name" id="name" required class="w-full px-4 py-4 border-2 border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:border-[#d71418] focus:ring-4 focus:ring-[#d71418]/10 transition-all duration-200" placeholder="Enter district name">
                    </div>
...
document.getElementById('addDistrictModal').addEventListener('click', function(e) {
    if (e.target === this) {
        closeAddModal();
    }
});
