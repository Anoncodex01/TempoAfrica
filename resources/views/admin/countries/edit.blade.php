...
<main class="flex-1 p-6 md:p-10 bg-gradient-to-br from-gray-50 to-gray-100">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-4xl font-bold text-gray-900 mb-2">Edit Country</h1>
                <p class="text-gray-600">Update country information and settings</p>
            </div>
            <a href="{{ route('admin.countries.index') }}" class="inline-flex items-center px-6 py-3 bg-white border border-gray-200 rounded-xl text-gray-700 font-medium hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 shadow-sm">
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                ...
                <div class="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-[#d71418]/5 to-[#f19e00]/5">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h2 class="text-xl font-semibold text-gray-900">Country Details</h2>
                            <p class="text-gray-600 text-sm">Modify the country information below</p>
                        </div>
                    </div>
                </div>

                <!-- Form Content -->
                <form action="{{ route('admin.countries.update', $country) }}" method="POST" class="p-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-8">
                        ...
                        <div class="relative">
                            <input 
                                ...
                                required 
                                class="w-full px-4 py-4 border-2 border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:border-[#d71418] focus:ring-4 focus:ring-[#d71418]/10 transition-all duration-200 bg-white"
                                placeholder="e.g., 255, US, GB"
                            >
                            <div class="absolute inset-y-0 right-0 flex items-center pr-4">
                                <i class="fas fa-check-circle text-green-500 opacity-0 transition-opacity duration-200" id="code-check"></i>
                            </div>
                        </div>
                    </div>
                        ...
                        <label class="flex items-center p-4 border-2 border-gray-200 rounded-xl hover:border-[#d71418]/30 transition-all duration-200 cursor-pointer group">
                            <div class="relative">
                                <input 
                                    ...
                                    id="is_established" 
                                    value="1" 
                                    {{ $country->is_established ? 'checked' : '' }} 
                                    class="sr-only"
                                >
                    ...
                    <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-100">
                        <a href="{{ route('countries.index') }}" class="px-8 py-3 bg-gray-100 text-gray-700 font-semibold rounded-xl hover:bg-gray-200 transition-all duration-200">
                            Cancel
                        </a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    codeInput.addEventListener('input', function() {
        if (this.value.length > 0) {
            codeCheck.style.opacity = '1';
        } else {
            codeCheck.style.opacity = '0';
        }
    });
