...
<main class="flex-1 p-6 md:p-10 bg-gradient-to-br from-gray-50 to-gray-100">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-4xl font-bold text-gray-900 mb-2">Provinces</h1>
                <p class="text-gray-600">Manage and organize province information</p>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div class="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-[#d71418]/5 to-[#f19e00]/5">
            <div class="flex items-center">
                ...
                <div>
                    <h2 class="text-xl font-semibold text-gray-900">Province List</h2>
                    <p class="text-gray-600 text-sm">View and manage all provinces in the system</p>
                </div>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-8 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Country</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-all duration-200">
                        <td class="px-8 py-6 whitespace-nowrap text-sm font-medium text-gray-900">{{ $province->id }}</td>
                        ...
                        <td class="px-8 py-6 whitespace-nowrap text-sm text-gray-600">{{ $province->country->name ?? 'N/A' }}</td>
                        <td class="px-8 py-6 whitespace-nowrap text-sm text-gray-600">{{ $province->zip_code ?? 'NULL' }}</td>
                        <td class="px-8 py-6 whitespace-nowrap">
                            @if($province->is_established)
                        ...
                        <td class="px-8 py-6 whitespace-nowrap text-sm text-gray-600">{{ $province->created_at ? $province->created_at->format('Y-m-d H:i:s') : 'N/A' }}</td>
                        <td class="px-8 py-6 whitespace-nowrap">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('admin.provinces.edit', $province) }}" class="inline-flex items-center px-4 py-2 bg-[#d71418] text-white text-xs font-semibold rounded-lg hover:bg-[#b31216] transition-all duration-200 shadow-sm hover:shadow-md">
                                    <i class="fas fa-edit mr-1"></i> Edit
                                </a>
                                <form method="POST" action="{{ route('admin.provinces.destroy', $province) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirm('Are you sure you want to delete this province?')" class="inline-flex items-center px-4 py-2 bg-gray-500 text-white text-xs font-semibold rounded-lg hover:bg-gray-600 transition-all duration-200 shadow-sm hover:shadow-md">
                        ...
                        <td colspan="7" class="px-8 py-12 text-center">
                            <div class="flex flex-col items-center">
                                <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mb-4">
                                    <i class="fas fa-map text-gray-400 text-xl"></i>
                                </div>
                                <p class="text-gray-500 font-medium">No provinces found</p>
                                <p class="text-gray-400 text-sm mt-1">Get started by adding your first province</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
...
<!-- Add Province Modal -->
<div id="addProvinceModal" class="fixed inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
        ...
        <div class="mt-3">
            ...
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center">
                    ...
                    <h3 class="text-xl font-semibold text-gray-900">Add New Province</h3>
                </div>
            ...
            <form action="{{ route('admin.provinces.store') }}" method="POST">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" required class="w-full px-4 py-4 border-2 border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:border-[#d71418] focus:ring-4 focus:ring-[#d71418]/10 transition-all duration-200" placeholder="Enter province name">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="country_id" id="country_id" required class="w-full px-4 py-4 border-2 border-gray-200 rounded-xl text-gray-900 focus:border-[#d71418] focus:ring-4 focus:ring-[#d71418]/10 transition-all duration-200">
...
function closeAddModal() {
    document.getElementById('addProvinceModal').classList.add('hidden');
}

// Close modal when clicking outside
document.getElementById('addProvinceModal').addEventListener('click', function(e) {
    if (e.target === this) {
        closeAddModal();
    }
});
