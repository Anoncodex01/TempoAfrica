...
<main class="flex-1 p-6 md:p-10 bg-gradient-to-br from-gray-50 to-gray-100">
    <div class="mb-8 flex items-center justify-between">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Add Room</h1>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 max-w-2xl mx-auto">
        <form action="{{ route('admin.houses.rooms.store', $house) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-gray-700 font-semibold mb-2">Name</label>
                    <input type="text" name="name" value="{{ old('name') }}" class="w-full px-4 py-2 border-2 border-gray-200 rounded-xl text-gray-900">
                    @error('name')<div class="text-red-500 text-xs mt-1">{{ $message }}</div>@enderror
                </div>
                <div>
                    <label class="block text-gray-700 font-semibold mb-2">Unique Number</label>
                    <input type="text" name="unique_number" value="{{ old('unique_number') }}" class="w-full px-4 py-2 border-2 border-gray-200 rounded-xl text-gray-900">
                ...
                <div>
                    ...
                    <input type="text" name="currency" value="{{ old('currency', 'USD') }}" class="w-full px-4 py-2 border-2 border-gray-200 rounded-xl text-gray-900">
                    @error('currency')<div class="text-red-500 text-xs mt-1">{{ $message }}</div>@enderror
                </div>
                <div>
                    <label class="block text-gray-700 font-semibold mb-2">Price</label>
                    <input type="number" step="0.01" name="price" value="{{ old('price') }}" class="w-full px-4 py-2 border-2 border-gray-200 rounded-xl text-gray-900">
                ...
                <div>
                    ...
                    <input type="text" name="price_duration" value="{{ old('price_duration') }}" class="w-full px-4 py-2 border-2 border-gray-200 rounded-xl text-gray-900">
                    @error('price_duration')<div class="text-red-500 text-xs mt-1">{{ $message }}</div>@enderror
                </div>
                <div>
                    <label class="block text-gray-700 font-semibold mb-2">Category</label>
                    <input type="text" name="category" value="{{ old('category') }}" class="w-full px-4 py-2 border-2 border-gray-200 rounded-xl text-gray-900">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="is_visible" value="1" @if(old('is_visible', true)) checked @endif>
            ...
            <div class="flex justify-end gap-4">
                ...
                <button type="submit" class="px-6 py-2 bg-gradient-to-r from-[#d71418] to-[#f19e00] text-white font-semibold rounded-xl shadow-md hover:from-[#b31216] hover:to-[#d68900] transition-all duration-200">Create Room</button>
            </div>
        </form>
    </div>
</main>
