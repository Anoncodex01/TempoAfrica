...
<main class="flex-1 p-6 md:p-10 bg-gradient-to-br from-gray-50 to-gray-100">
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 mb-2">Room Photos</h1>
            <p class="text-gray-600">Manage photos for <span class="font-semibold">{{ $room->name }}</span></p>
        </div>
        <a href="{{ route('admin.houses.rooms.show', [$room->house_id, $room]) }}" class="inline-flex items-center px-6 py-3 bg-gray-100 text-gray-700 font-semibold rounded-xl hover:bg-gray-200 transition-all duration-200">
            <i class="fas fa-arrow-left mr-2"></i> Back to Room
        </a>
    </div>
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 mb-8">
        <form action="{{ route('admin.rooms.photos.store', $room) }}" method="POST" enctype="multipart/form-data" class="flex flex-col md:flex-row items-center gap-4">
            @csrf
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
        ...
        <div class="bg-gray-50 rounded-xl shadow p-4 flex flex-col items-center">
            ...
            <div class="flex items-center gap-2">
                <form action="{{ route('admin.rooms.photos.toggle', [$room, $photo]) }}" method="POST">
                    @csrf
                ...
                <form action="{{ route('admin.rooms.photos.destroy', [$room, $photo]) }}" method="POST" onsubmit="return confirm('Delete this photo?');">
                    @csrf
                    @method('DELETE')
