...
<main class="flex-1 p-4 md:p-6 bg-gradient-to-br from-gray-50 to-gray-100">
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-gray-900 mb-1">Edit Accommodation</h1>
                <p class="text-sm text-gray-600">Update accommodation information</p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-[#d71418]/5 to-[#f19e00]/5">
            <div class="flex items-center">
                ...
                <div>
                    <h2 class="text-lg font-semibold text-gray-900">Edit Accommodation Information</h2>
                    <p class="text-xs text-gray-600">Update the details below to modify this accommodation</p>
                </div>
            </div>
        </div>

        <form method="POST" action="{{ route('admin.accommodations.update', $accommodation->id) }}" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category }}" {{ old('category', $accommodation->category) == $category ? 'selected' : '' }}>
                                    {{ $category }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            value="{{ old('registration_number', $accommodation->registration_number) }}"
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            id="unique_number"
                            name="unique_number" 
                            value="{{ old('unique_number', $accommodation->unique_number) }}"
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($customers as $customer)
                                <option value="{{ $customer->id }}" {{ old('customer_id', $accommodation->customer_id) == $customer->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($provinces as $province)
                                <option value="{{ $province->id }}" {{ old('province_id', $accommodation->province_id) == $province->id ? 'selected' : '' }}>
                                    {{ $province->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($streets as $street)
                                <option value="{{ $street->id }}" {{ old('street_id', $accommodation->street_id) == $street->id ? 'selected' : '' }}>
                                    {{ $street->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            id="minimum_price"
                            name="minimum_price" 
                            value="{{ old('minimum_price', $accommodation->minimum_price) }}"
                            placeholder="0.00"
                            step="0.01"
                            min="0"
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option value="USD" {{ old('currency', $accommodation->currency) == 'USD' ? 'selected' : '' }}>USD - US Dollar</option>
                            <option value="EUR" {{ old('currency', $accommodation->currency) == 'EUR' ? 'selected' : '' }}>EUR - Euro</option>
                            <option value="GBP" {{ old('currency', $accommodation->currency) == 'GBP' ? 'selected' : '' }}>GBP - British Pound</option>
                            <option value="KES" {{ old('currency', $accommodation->currency) == 'KES' ? 'selected' : '' }}>KES - Kenyan Shilling</option>
                            <option value="UGX" {{ old('currency', $accommodation->currency) == 'UGX' ? 'selected' : '' }}>UGX - Ugandan Shilling</option>
                            <option value="TZS" {{ old('currency', $accommodation->currency) == 'TZS' ? 'selected' : '' }}>TZS - Tanzanian Shilling</option>
                        </select>
                        @error('currency')
                            <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option value="per_night" {{ old('minimum_price_duration', $accommodation->minimum_price_duration) == 'per_night' ? 'selected' : '' }}>Per Night</option>
                            <option value="per_week" {{ old('minimum_price_duration', $accommodation->minimum_price_duration) == 'per_week' ? 'selected' : '' }}>Per Week</option>
                            <option value="per_month" {{ old('minimum_price_duration', $accommodation->minimum_price_duration) == 'per_month' ? 'selected' : '' }}>Per Month</option>
                        </select>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center">
                            <input 
                                ...
                                name="is_visible" 
                                value="1"
                                {{ old('is_visible', $accommodation->is_visible) ? 'checked' : '' }}
                                class="w-4 h-4 text-[#d71418] border-gray-300 rounded focus:ring-[#d71418] focus:ring-2"
                            >
                        ...
                        <div class="flex items-center">
                            <input 
                                ...
                                name="is_approved" 
                                value="1"
                                {{ old('is_approved', $accommodation->is_approved) ? 'checked' : '' }}
                                class="w-4 h-4 text-[#d71418] border-gray-300 rounded focus:ring-[#d71418] focus:ring-2"
                            >
                ...
                <div class="space-y-4">
                    <h3 class="text-sm font-semibold text-gray-900 border-b border-gray-200 pb-2">Current Photos</h3>
                    
                    <div class="grid grid-cols-1 gap-3">
                        @if($accommodation->photo)
                        <div class="relative group">
                            <img src="{{ Storage::url($accommodation->photo) }}" alt="Main Accommodation Photo" class="w-full h-32 object-cover rounded-lg">
                            <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-200 rounded-lg flex items-center justify-center">
                                <span class="text-white opacity-0 group-hover:opacity-100 transition-opacity duration-200 text-xs font-medium">Main Photo</span>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="relative group">
                            <img src="{{ Storage::url($photo->photo) }}" alt="{{ $photo->description ?? 'Accommodation Photo' }}" class="w-full h-32 object-cover rounded-lg">
                            <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-200 rounded-lg flex items-center justify-center">
                                <span class="text-white opacity-0 group-hover:opacity-100 transition-opacity duration-200 text-xs font-medium">{{ $photo->description ?? 'Additional Photo' }}</span>
                            </div>
                        </div>
                        @endforeach
                    </div>
            ...
            <div class="mt-6">
                ...
                <div class="mb-4">
                    ...
                    >
                    <p class="text-xs text-gray-500 mt-1">Upload a new main photo (max 2MB)</p>
                    @error('photo')
                        <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex items-center justify-end space-x-3 mt-8 pt-6 border-t border-gray-100">
                <a href="{{ route('admin.accommodations.show', $accommodation) }}" class="inline-flex items-center px-4 py-2 bg-gray-100 text-gray-700 text-sm font-semibold rounded-lg hover:bg-gray-200 transition-all duration-200">
